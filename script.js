...
/* ----- Degradation chart (Chart.js) — LIGHT MODE 2026-08-22 v3, REVERT 2026-08-23 -----
   2026-08-23: revert to single canvas + 3 tabs (years / km / cycles).
   Each tab swaps the dataset on the same chart; the chart always shows 3 lines
   (NCA, NCM811, LFP). Same UX as the kalkulator.html tabbed version.
*/
(function initDegradationChart() {
    ...
    const data = {
        ...
        cycles: {
            ...
            datasets: [
                { label: "NCA (tipikus ciklusszám)", data: [100, 98, 95, 92, 88, 84, 80, 76, 72], borderColor: "#8b5cf6", backgroundColor: "rgba(139, 92, 246, 0.15)", pointBackgroundColor: "#8b5cf6" },
    ...
    // Same point/line defaults across all three datasets in all three modes.
    ...
    const baseOptions = {
        ...
        plugins: {
            ...
            // clamp:true keeps labels inside the chart canvas so they don't spill.
            datalabels: {
            ...
            tooltip: {
                ...
                titleColor: "#ffffff", bodyColor: "#e0f2fe",
        ...
        scales: {
            y: {
                min: 60, max: 102,
                ticks: { color: "#1a1a26", font: { family: "Inter", size: 15, weight: "700" }, padding: 8, callback: (v) => `${v}%` },
                grid: { color: "rgba(8, 47, 73, 0.10)" },
                border: { color: "rgba(8, 47, 73, 0.35)", display: true, width: 2 },
                title: { display: true, text: "Maradék kapacitás (%)", color: "#020a14", font: { family: "Inter Tight", size: 16, weight: "800" }, padding: { bottom: 12 } },
            },
            x: {
                ticks: { color: "#1a1a26", font: { family: "Inter", size: 14, weight: "700" }, padding: 8 },
                grid: { color: "rgba(8, 47, 73, 0.06)" },
                border: { color: "rgba(8, 47, 73, 0.35)", display: true, width: 2 },
                title: { display: true, color: "#020a14", font: { family: "Inter Tight", size: 16, weight: "800" }, padding: { top: 12 } },
            },
        },
...
(function initKalkDegChart() {
    ...
    const baseOptions = {
        ...
        plugins: {
            ...
            // clamp:true keeps labels inside the chart area.
            datalabels: {
                display: (ctx) => ctx.dataIndex % 2 === 0,
                anchor: "end",
                align: "top",
